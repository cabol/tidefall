defmodule Tidefall.CoalescedMapTest do
  use ExUnit.Case, async: false

  alias Tidefall.CoalescedMap
  alias Tidefall.CoalescedMap.Entry
  ...
  describe "stop/3" do
    test "ok: stops the buffer" do
      {:ok, _} =
        CoalescedMap.start_link(
          name: :cm_stop_buff,
          processor: &__MODULE__.test_processor(self(), &1),
          partitions: 1
        )

      assert CoalescedMap.stop(:cm_stop_buff) == :ok

      assert_receive {@partition_stop_event, %{duration: _},
                      %{buffer: :cm_stop_buff, partition: _, reason: _}},
                     @default_timeout
    end
  end
  ...
  describe "put/4 and put_all/3" do
    ...
    test "ok: single put is processed", %{buffer: buff} do
      assert CoalescedMap.size(buff) == 0

      assert CoalescedMap.put(buff, :key1, "value1") == :ok

      assert CoalescedMap.size(buff) == 1
      assert CoalescedMap.get(buff, :key1) == "value1"
    ...
    test "ok: batch put with list of tuples", %{buffer: buff} do
      ...
      assert CoalescedMap.put_all(buff, entries) == :ok

      assert CoalescedMap.size(buff) == 3
    ...
    test "ok: batch put with map", %{buffer: buff} do
      ...
      assert CoalescedMap.size(buff) == 2
    ...
    test "ok: last-write-wins for duplicate keys", %{buffer: buff} do
      assert CoalescedMap.put(buff, :dup, "first") == :ok
      assert CoalescedMap.get(buff, :dup) == "first"

      assert CoalescedMap.put(buff, :dup, "second") == :ok
      assert CoalescedMap.get(buff, :dup) == "second"
    ...
    test "ok: entries are partitioned using a function", %{buffer: buff} do
      assert CoalescedMap.put(buff, :key1, "value1", partition_key: &__MODULE__.partition_key_fun/1) ==
               :ok
    ...
    test "ok: entries are partitioned using an MFA tuple", %{buffer: buff} do
      assert CoalescedMap.put(buff, :key1, "value1",
    ...
    test "ok: entries are partitioned using a static key", %{buffer: buff} do
      assert CoalescedMap.put(buff, :key1, "value1", partition_key: :static) == :ok
    ...
    test "error: no partitions available for the given buffer" do
      ...
      assert_raise RuntimeError, ~r"no partitions available for buffer :cm_error_buff", fn ->
        CoalescedMap.put(:cm_error_buff, :key, "value")
      end

      assert CoalescedMap.stop(buff) == :ok
    end
  end
  ...
  describe "get/3" do
    setup do
      ...
      pid =
        start_supervised!(
          {CoalescedMap, name: __MODULE__, processor: &__MODULE__.test_processor(self, &1)}
        )
    ...
    test "ok: returns value for existing key", %{buffer: buff} do
      :ok = CoalescedMap.put(buff, :key1, "value1")

      assert CoalescedMap.get(buff, :key1) == "value1"
    end

    test "ok: returns nil for missing key", %{buffer: buff} do
      assert CoalescedMap.get(buff, :missing) == nil
    end

    test "ok: returns custom default for missing key", %{buffer: buff} do
      assert CoalescedMap.get(buff, :missing, :not_found) == :not_found
    end

    test "ok: reflects latest value after overwrite", %{buffer: buff} do
      :ok = CoalescedMap.put(buff, :key1, "v1")
      assert CoalescedMap.get(buff, :key1) == "v1"

      :ok = CoalescedMap.put(buff, :key1, "v2")
      assert CoalescedMap.get(buff, :key1) == "v2"
    end
  end
  ...
  describe "delete/3" do
    ...
    test "ok: put then delete before processing", %{buffer: buff} do
      assert CoalescedMap.put(buff, :to_delete, "gone") == :ok
      assert CoalescedMap.put(buff, :to_keep, "stays") == :ok
      assert CoalescedMap.size(buff) == 2

      # Verify both exist before delete
      assert CoalescedMap.get(buff, :to_delete) == "gone"
      assert CoalescedMap.get(buff, :to_keep) == "stays"

      assert CoalescedMap.delete(buff, :to_delete) == :ok
      assert CoalescedMap.size(buff) == 1

      # Verify deleted key is gone and kept key remains
      assert CoalescedMap.get(buff, :to_delete) == nil
      assert CoalescedMap.get(buff, :to_keep) == "stays"
  ...
  describe "put_newer/4 and put_all_newer/3" do
    ...
    test "ok: inserts new entry", %{buffer: buff} do
      assert CoalescedMap.put_newer(buff, :key1, "value1", version: 100) == :ok
    ...
    test "ok: defaults version to a positive monotonic integer when omitted", %{buffer: buff} do
      assert CoalescedMap.put_newer(buff, :key1, "value1") == :ok
    ...
    test "ok: newer version wins", %{buffer: buff} do
      assert CoalescedMap.put_newer(buff, :key1, "v1", version: 100) == :ok
      assert CoalescedMap.get(buff, :key1) == "v1"

      # Newer version should overwrite
      assert CoalescedMap.put_newer(buff, :key1, "v2", version: 200) == :ok
      assert CoalescedMap.get(buff, :key1) == "v2"
    ...
    test "ok: older version is ignored", %{buffer: buff} do
      assert CoalescedMap.put_newer(buff, :key1, "v1", version: 200) == :ok
      assert CoalescedMap.get(buff, :key1) == "v1"

      # Older version should be ignored
      assert CoalescedMap.put_newer(buff, :key1, "v2", version: 100) == :ok
      assert CoalescedMap.get(buff, :key1) == "v1"
    ...
    test "ok: put_all_newer with multiple entries", %{buffer: buff} do
      ...
      assert CoalescedMap.put_all_newer(buff, entries) == :ok

      assert CoalescedMap.size(buff) == 3
      assert CoalescedMap.get(buff, :a) == "val_a"
      assert CoalescedMap.get(buff, :b) == "val_b"
      assert CoalescedMap.get(buff, :c) == "val_c"
    ...
    test "ok: put_all_newer respects version ordering", %{buffer: buff} do
      ...
      assert CoalescedMap.put_all_newer(buff, initial) == :ok
      ...
      assert CoalescedMap.put_all_newer(buff, updates) == :ok

      # :a should be updated (200 > 100), :b should not (100 < 200)
      assert CoalescedMap.get(buff, :a) == "a2"
      assert CoalescedMap.get(buff, :b) == "b1"
    ...
    test "ok: works with timestamp versions", %{buffer: buff} do
      ...
      assert CoalescedMap.put_newer(buff, :event, %{data: "first"}, version: t1) == :ok

      t2 = System.monotonic_time()
      assert CoalescedMap.put_newer(buff, :event, %{data: "second"}, version: t2) == :ok

      # t2 > t1, so second should win
      assert CoalescedMap.get(buff, :event) == %{data: "second"}
    ...
    test "ok: updates existing entry with tuple key and higher version", %{buffer: buff} do
      assert CoalescedMap.put_newer(buff, {1, "key1"}, {"value0"}, version: 100) == :ok
      assert CoalescedMap.put_newer(buff, {1, "key1"}, {"value1"}, version: 200) == :ok

      assert CoalescedMap.size(buff) == 1
      assert CoalescedMap.get(buff, {1, "key1"}) == {"value1"}
    ...
    test "ok: updates existing entry with list key and list value containing tuples",
      ...
      assert CoalescedMap.put_newer(buff, key, value0, version: 100) == :ok
      assert CoalescedMap.put_newer(buff, key, value1, version: 200) == :ok

      assert CoalescedMap.size(buff) == 1
      assert CoalescedMap.get(buff, key) == value1
    ...
    test "ok: updates existing entry with tuple value containing maps", %{buffer: buff} do
      ...
      assert CoalescedMap.put_newer(buff, :tuple_map, value0, version: 100) == :ok
      assert CoalescedMap.put_newer(buff, :tuple_map, value1, version: 200) == :ok

      assert CoalescedMap.size(buff) == 1
      assert CoalescedMap.get(buff, :tuple_map) == value1
    ...
    test "ok: accepts binary and atom versions (compared via Erlang term order)",
         %{buffer: buff} do
      assert CoalescedMap.put_newer(buff, :str_key, "v1", version: "01HXYZ-001") == :ok
      assert CoalescedMap.put_newer(buff, :str_key, "v2", version: "01HXYZ-002") == :ok
      assert CoalescedMap.put_newer(buff, :str_key, "v_old", version: "01HXYZ-000") == :ok

      assert CoalescedMap.put_newer(buff, :atom_key, "v1", version: :a) == :ok
      assert CoalescedMap.put_newer(buff, :atom_key, "v2", version: :b) == :ok

      assert CoalescedMap.size(buff) == 2
      assert CoalescedMap.get(buff, :str_key) == "v2"
      assert CoalescedMap.get(buff, :atom_key) == "v2"
    end
    ...
    test "error: put_newer raises NimbleOptions.ValidationError for unsupported version type",
      ...
      assert_raise NimbleOptions.ValidationError, ~r/:version option/, fn ->
        CoalescedMap.put_newer(buff, :key1, "value1", version: {1, 5})
      end
    end
    ...
    test "error: put_newer raises NimbleOptions.ValidationError for invalid :key_hasher",
      ...
      assert_raise NimbleOptions.ValidationError, ~r/:key_hasher option/, fn ->
        CoalescedMap.put_newer(buff, :key1, "value1", key_hasher: :not_a_function)
      end
    end
    ...
    test "error: put_all_newer raises ArgumentError for invalid entry",
      ...
      assert_raise ArgumentError, ~r/invalid entry/, fn ->
        CoalescedMap.put_all_newer(buff, entries)
      end
    end
  end
  ...
  describe "key_hasher option" do
    ...
    test "ok: put + get + delete round-trip with key_hasher: true", %{buffer: buff} do
      ...
      assert CoalescedMap.put(buff, key, "v1", key_hasher: true) == :ok
      assert CoalescedMap.get(buff, key, nil, key_hasher: true) == "v1"
      assert CoalescedMap.delete(buff, key, key_hasher: true) == :ok
      assert CoalescedMap.get(buff, key, nil, key_hasher: true) == nil
    end

    test "ok: put_newer round-trip with key_hasher: true and map keys", %{buffer: buff} do
      key = %{tenant: "acme", id: 42}

      assert CoalescedMap.put_newer(buff, key, "v1", version: 100, key_hasher: true) == :ok
      assert CoalescedMap.put_newer(buff, key, "v2", version: 200, key_hasher: true) == :ok
      # Older version is rejected by the match-spec guard
      assert CoalescedMap.put_newer(buff, key, "v_old", version: 50, key_hasher: true) == :ok

      assert CoalescedMap.size(buff) == 1
      assert CoalescedMap.get(buff, key, nil, key_hasher: true) == "v2"
    end
    ...
    test "ok: put_all_newer with key_hasher: true", %{buffer: buff} do
      ...
      assert CoalescedMap.put_all_newer(
               buff,
      ...
      assert CoalescedMap.size(buff) == 2
      assert CoalescedMap.get(buff, key1, nil, key_hasher: hasher) == "v1"
      assert CoalescedMap.get(buff, key2, nil, key_hasher: hasher) == "v2"
    end
    ...
    test "ok: lambda hasher (sha256 over term_to_binary)", %{buffer: buff} do
      ...
      assert CoalescedMap.put(buff, key, "v1", key_hasher: hasher) == :ok
      assert CoalescedMap.get(buff, key, nil, key_hasher: hasher) == "v1"
    ...
    test "ok: non-hashed and hashed entries coexist in the same partition", %{buffer: buff} do
      ...
      assert CoalescedMap.put(buff, :plain, "v_plain") == :ok
      # With key_hasher: stored under the hash, raw_key holds the original
      assert CoalescedMap.put(buff, %{m: 1}, "v_map", key_hasher: true) == :ok

      assert CoalescedMap.size(buff) == 2
      assert CoalescedMap.get(buff, :plain) == "v_plain"
      assert CoalescedMap.get(buff, %{m: 1}, nil, key_hasher: true) == "v_map"
  ...
  describe "processing" do
    ...
    test "error: task fails", %{buffer: buff} do
      :ok = CoalescedMap.put(buff, :err, %{error: true})
    ...
    test "error: task exits", %{buffer: buff} do
      ...
      :ok = CoalescedMap.put(buff, :exit_key, %{exit: :exit})
    ...
    test "shutdown: entries are processed before dying", %{buffer: buff, pid: pid} do
      ...
      :ok = CoalescedMap.put(buff, :slow, %{sleep_ms: 500, data: "first"})
      ...
      :ok = CoalescedMap.put(buff, :fast, %{id: 2, data: "second"})
      ...
      # (CoalescedMap returns %Entry{} structs in the batch)
      assert_receive {:process_completed,
                      [
                        %Entry{
                          key: :slow,
  ...
  describe "update_options/2" do
    setup do
      ...
      start_supervised!(
        {CoalescedMap,
         name: __MODULE__, processor: &__MODULE__.test_processor(self, &1), partitions: 1}
      )
    ...
    test "ok: updates the given options", %{buffer: buff} do
      ...
      assert CoalescedMap.update_options(buff,
               processing_interval: 200,
               processing_batch_size: 2
             ) == :ok

      # Insert 4 entries
      assert CoalescedMap.put_all(buff, [{:a, 1}, {:b, 2}, {:c, 3}, {:d, 4}]) == :ok
    ...
    test "error: raises on invalid options", %{buffer: buff} do
      assert_raise NimbleOptions.ValidationError, fn ->
        CoalescedMap.update_options(buff, processing_interval: -1)
      end
    end
  end
  ...
  describe "processing_batch_size: :table" do
    setup do
      ...
      {:ok, buffer: :cm_table_test, pid: pid}
    end

    test "ok: processor receives the ETS table", %{buffer: buff} do
      assert CoalescedMap.put_all(buff, [{:a, 1}, {:b, 2}, {:c, 3}]) == :ok
    ...
    test "ok: processor can take ownership and keep the table", %{buffer: buff} do
      assert CoalescedMap.put_all(buff, [{:x, 10}, {:y, 20}]) == :ok
  ...
  describe "partitioning" do
    setup do
      ...
      {:ok, buffer: :cm_partitioning_test}
    end
  ...
  # CoalescedMap processor receives %Entry{} structs in the batch.
