# Benchmark: Tidefall.CoalescedMap operations
#
# Run with: mix run benchmarks/kv_bench.exs

alias Tidefall.CoalescedMap

IO.puts("Setting up CoalescedMap buffer for benchmarks...\n")

# Start the buffer with a no-op processor (we just want to measure write performance)
{:ok, _pid} =
  CoalescedMap.start_link(
    name: :bench_coalesced_map,
...
CoalescedMap.put_all(:bench_coalesced_map, entries)
...
Benchee.run(
  %{
    "put_all/3" => fn {batch, _} ->
      CoalescedMap.put_all(:bench_coalesced_map, batch)
    end,
    "put_all_newer/3" => fn {_, versioned_batch} ->
      CoalescedMap.put_all_newer(:bench_coalesced_map, versioned_batch)
    end
  },
  inputs: %{
...
# Cleanup
CoalescedMap.stop(:bench_coalesced_map)
