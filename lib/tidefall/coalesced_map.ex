defmodule Tidefall.CoalescedMap do
  @moduledoc """
  ETS-based coalescing key-value buffer for high-throughput data processing.

  `Tidefall.CoalescedMap` buffers key-value entries using `:set` ETS tables,
  ...
  #{Tidefall.CoalescedMap.Options.runtime_options_docs()}

  Additional options accepted by `put_newer/4`:

  #{Tidefall.CoalescedMap.Options.put_newer_options_docs()}

  ## Examples

  ### Standalone Usage

      # Start a CoalescedMap buffer with a custom processor
      iex> {:ok, _sup_pid} =
      ...>   Tidefall.CoalescedMap.start_link(
      ...>     name: :my_coalesced_map_buffer,
      ...>     processor: fn batch -> IO.inspect(batch) end
      ...>   )

      # Put a single entry
      iex> Tidefall.CoalescedMap.put(:my_coalesced_map_buffer, :key1, "value1")
      :ok

      # Put multiple entries at once
      iex> Tidefall.CoalescedMap.put_all(:my_coalesced_map_buffer, %{key2: "value2", key3: "value3"})
      :ok

      # Delete an entry
      iex> Tidefall.CoalescedMap.delete(:my_coalesced_map_buffer, :key1)
      :ok

      # Versioned put (newer version wins)
      iex> Tidefall.CoalescedMap.put_newer(:my_coalesced_map_buffer, :key4, "v1", version: 100)
      :ok
      iex> Tidefall.CoalescedMap.put_newer(:my_coalesced_map_buffer, :key4, "v2", version: 200)
      :ok
      iex> Tidefall.CoalescedMap.get(:my_coalesced_map_buffer, :key4)
      "v2"

      # Check buffer size
      iex> Tidefall.CoalescedMap.size(:my_coalesced_map_buffer)
      3

      # Stop the buffer gracefully (processes remaining items)
      iex> Tidefall.CoalescedMap.stop(:my_coalesced_map_buffer)
      :ok
  ...
  The processor function receives a list of `t:Tidefall.CoalescedMap.Entry.t/0`
  ...
  alias Tidefall.CoalescedMap.{Entry, Options}
  ...
  @typedoc """
  Key-hashing option accepted by every CoalescedMap operation:
  ...
  @doc """
  Starts a new CoalescedMap buffer.

  ## Options

  See [start options](`m:Tidefall.CoalescedMap#module-start-options`).

  ## Examples

      Tidefall.CoalescedMap.start_link(name: :my_coalesced_map_buffer)
  ...
  @doc """
  Stops a CoalescedMap buffer gracefully.

  ## Examples

      Tidefall.CoalescedMap.stop(:my_coalesced_map_buffer)
  ...
  @doc """
  Returns the CoalescedMap buffer child spec.
  """
  ...
  See [runtime options](`m:Tidefall.CoalescedMap#module-runtime-options`).
  ...
  @doc """
  Returns the CoalescedMap buffer size (total number of entries across all partitions).
  ...
  @doc """
  Updates the options for the CoalescedMap buffer.

  ## Options

  Updatable options: `:processing_interval`, `:processing_timeout`,
  `:processing_batch_size`. See [start options](`m:Tidefall.CoalescedMap#module-start-options`)
